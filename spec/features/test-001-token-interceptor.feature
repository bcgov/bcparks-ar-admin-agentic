Feature: Token interceptor unit coverage
  ...
  So that Bearer injection and 401 refresh/retry regressions are caught in CI

  # Finding: RA TEST-001 · Issue: #68
  # Assessment Expected: (1) Bearer when authenticated (2) no usable auth when unauthenticated
  #   (3) 401 → refresh + retry (4) refresh failure causes logout
  # Conscious scope: this slice is coverage of *current* interceptor behaviour.
  # AUTH-006 (401 vs 403) is now implemented; see auth-006-interceptor-401.feature.
  # Do not "fix" AUTH-007 (host allowlist) here.
  ...
  @R-04.3
  Scenario: HTTP 401 refreshes the token and retries
    Given an HTTP request that fails with 401
  ...
  @R-04.5
  Scenario: Non-401 errors pass through without refresh
    Given an HTTP request that fails with a status other than 401
    When the interceptor handles the error
    Then it does not refresh the token
    And the error is surfaced to the caller

  @R-04.6
  Scenario: Concurrent 401s share one in-flight refresh
    Given a token refresh is already in progress
    When another request fails with 401
